...
Definition t4 : Prop := forall (A : Prop), A -> A.
...
(* Exercise: connectives [4 star]. 
State and prove all of the following as theorems.  Some
of them duplicate theorems proved above, but we repeat
them here because together these theorems completely
characterize the interaction between the various
logical connecctives.
  (A -> B) -> ((B -> C) -> (A -> C))
  A -> (A \/ B)
  B -> (A \/ B)
  (A -> C) -> ((B -> C) -> ((A \/ B) -> C))
